...
export const createCabBooking = async (bookingData) => {
	try {
		...
		// saving booking history to user
		const user = await User.findById(userId);
		user.bookingHistory.push({
			cabBookingId: newCabBooking._id,
			travellerName: newCabBooking.travellerName,
			travellerAge: newCabBooking.travellerAge,
			travellerGender: newCabBooking.travellerGender,
			travellerEmail: newCabBooking.travellerEmail,
			travellerPhoneNumber: newCabBooking.travellerPhoneNumber,
			pickupLocation: newCabBooking.pickupLocation,
			dropoffLocation: newCabBooking.dropoffLocation,
			pickupDateTime: newCabBooking.pickupDateTime,
			totalAmount: newCabBooking.totalAmount,
			status: newCabBooking.status,
		});
		await user.save();
...
export const cancelCabBooking = async (bookingId) => {
	try {
		const canceledBooking = await CabBooking.findByIdAndUpdate(bookingId, {
			status: "canceled",
		});
